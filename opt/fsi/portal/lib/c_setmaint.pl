any [ 'get', 'post' ] => '/setmaintenancemodemark' => sub {
   my $weburl = '/setmaintenancemodemark';
   session 'now' => $weburl;
      ...
      if ( request->method() eq "POST" ) {
         ...
         if ( params->{'OK'} ) {
            ...
            foreach my $srvid ( keys %$marksrvhash ) {
               ...
               $logger->info("$ll  set maintenance mode server $server");
               $retc = setmaintenancemode($server);
               if ($retc) {
                  $logger->error("setting srv $server in maintenance mode");
                  $errmsg = $server;
                  last;
               }
            ...
            unless ($retc) {
               if ( $srvcount > 1 ) {
                  set_flash("!S:Setting multiple server maintenance mode successful: $srvnames");
               } else {
                  set_flash("!S:Setting server maintenance mode successful: $srvnames");
               }
            } else {
               if ( $srvcount > 1 ) {
                  set_flash("!E:ERROR setting multiple server maintenance mode: $errmsg");
               } else {
                  set_flash("!E:ERROR setting server maintenance mode: $errmsg");
               }
            } ## end else
         } else {
            if ( $srvcount > 1 ) {
               set_flash("!W:Abort setting multiple server maintenance mode: $srvnames");
            } else {
               set_flash("!W:Abort setting server maintenance mode: $srvnames");
            }
         } ## end else [ if ( params->{'OK'} ) ]
         redirect $sess_back;
      } else {
         my $sess_reload = "/setmaintenancemodemark";
         ...
         template 'mark/setmaintenancemode',
           {
             'version' => $ver,
             'vitemp'  => $host,
             'vienv'   => $global{'vienv'},
