#!/usr/bin/perl -w
# 
#   dbnet2vim.pl - send email for dbnet2vi
#   Copyright (C) 2012 brandom zehm + js
#
...
my %conf = (
    ...
    "version"              => '2.04',                              ## The version of this program
...
my $conf_file = $dirs . '../etc/log4p_fsic';
...
for ($counter = 0; $counter < $numargv; $counter++) {
  if ($ARGS[$counter] =~ /^-l$/) {                  ## Logging ##
     $counter++;
     $conf{'logging'} = 1;
     if ($ARGS[$counter] && $ARGS[$counter] !~ /^-/) { 
        $logfile = $ARGS[$counter]; 
      } else { 
        print ("\n\nERROR: The argument after -l was not the log file!\n\n"); 
        $counter--; 
     }
  }
}
if ($logfile eq "none") {
   $logfile = sprintf "%s../logs/%s.log", $dirs, $prgname; 
}
sub log4p_logfile { return  $logfile };
Log::Log4perl->init( $conf_file );
my $logger = Log::Log4perl::get_logger();
$logger->info("Starting $conf{'programName'} - version $conf{'version'}");
...
$logger->debug("Logfile: $conf{'logFile'}");
...
sub processCommandLine {
    ...
    for ($counter = 0; $counter < $numargv; $counter++) {
        ...
        if ($ARGS[$counter] =~ /^-h$/i) {                    ## Help ##
            ...
            while ($ARGS[$counter] && $ARGS[$counter] !~ /^-/) {
                ...
                if ($ARGS[$counter] !~ /(\S+)=(\S.*)/) {
                    $logger->warn("Name/Value pair [$ARGS[$counter]] is not properly formatted");
                    $logger->warn("Arguments proceeding -o should be in the form of \"name=value\"");
                }
                else {
                    if (exists($opt{$1})) {
                        ...
                        $logger->debug("Assigned \$opt{} key/value: $1 => $2");
                    }
                    else {
                        $logger->warn("Name/Value pair [$ARGS[$counter]] will be ignored: unknown key [$1]");
                        $logger->info("Try the --help option to find valid command line arguments");
                    }
                }
                $counter++;
            }   $counter--;
            ...
            else { $logger->warn("The argument after -f was not an email address!"); $counter--; }
        }
            ...
            else { $logger->warn("The argument after -s was not the server!"); $counter--; }
        }
            ...
            else { $logger->warn("The argument after -b was not the bindaddr!"); $counter--; }
        }
            ...
            if ($ARGS[$counter] && $ARGS[$counter] !~ /^-/) {
               $opt{'username'} = $ARGS[$counter];
                ...
                $logger->warn("The argument after -xu was not valid username!");
                $counter--;
            }
        }
            ...
            if ($ARGS[$counter] && $ARGS[$counter] !~ /^-/) {
               $opt{'password'} = $ARGS[$counter];
                ...
                $logger->warn("The argument after -xp was not valid password!");
                $counter--;
            }
        }
        ...
        elsif ($ARGS[$counter] =~ /^-l$/) {                  ## Logging - jump over we check it already ##
           $counter++;
           if ($ARGS[$counter] && $ARGS[$counter] !~ /^-/) { 
           } else { 
              $counter--; 
            ...
            $logger->error("\"$ARGS[$counter]\" is not a recognized option!");
            help();
        }
    ...
    foreach my $addr (@to, @cc, @bcc, $from, $opt{'reply-to'}) {
        if ($addr) {
            if (!returnAddressParts($addr)) {
                $logger->error("Can't use improperly formatted email address: $addr");
                $logger->info("Try viewing the extended help on addressing with \"--help addressing\"");
                quit("", 1);
            }
        }
    }
    ...
    foreach my $file (@attachments) {
        if ( (! -f $file) or (! -r $file) ) {
            $logger->error("The attachment [$file] doesn't exist!");
            $logger->info("Try specifying the full path to the file or reading extended help with \"--help message\"");
            quit("", 1);
        }
    }
...
sub SMTPchat {
    ...
    $logger->debug("Sending: \t$command") if ($command);
...
sub evalSMTPresponse {
    ...
    $logger->trace("evalSMTPresponse() - Checking for SMTP success or error status in the message: $message ");
    
    ## Look for a SMTP success code
    if ($message =~ /^([23]\d\d)/) {
        $logger->debug("evalSMTPresponse() - Found SMTP success code: $1");
    ...
    if ($message =~ /^([45]\d\d)/) {
        $logger->debug("evalSMTPresponse() - Found SMTP error code: $1");
...
sub returnAddressParts {
    ...
    unless ($name and $address) {
        $logger->error("Couldn't parse the address: $input");
        $logger->info("If you think this should work, consider reporting this as a bug");
        return(undef());
    }
    ...
    if ($address =~ /[<> ,;:"'\[\]\\$ctrl$nonASCII]/) {
        $logger->warn("The address [$address] seems to contain invalid characters: continuing anyway");
    }
    return($name, $address);
}
...
sub send_attachment {
  ...
  ############################
    ...
    open (FILETOATTACH, $filename) || do {
        $logger->error("Opening the file [$filename] for attachment failed with the error: $!");
        return(1);
    };
    ...
    close (FILETOATTACH) || do {
        $logger->error("Closing the filehandle for file [$filename] failed with the error: $!");
        return(2);
    };
...
sub get_hostname {
    ...
    ## If that didn't get us a hostname, try a few other things
        ...
        if (!$hostname) {
            print "No hostname could be determined, please specify one with -o fqdn=FQDN option!\n";
            return("unknown");
        }
    }
...
${colorBold}$conf{'programName'}-$conf{'version'} ${colorNoBold}
...
if (!($message)) {
    ...
    if ($opt{'message-file'}) {
        if (! -e $opt{'message-file'}) {
            $logger->error("Message body file specified [$opt{'message-file'}] does not exist!");
            $logger->info("1) check spelling of your file; 2) fully qualify the path; 3) doubble quote it");
            quit("", 1);
        }
        if (! -r $opt{'message-file'}) {
            $logger->error("Message body file specified can not be read due to restricted permissions!");
            $logger->info("Check permissions on file specified to ensure it can be read");
            quit("", 1);
        }
        if (!open(MFILE, "< " . $opt{'message-file'})) {
            $logger->error("Error opening message body file [$opt{'message-file'}]: $!");
            quit("", 1);
        }
        while (<MFILE>) {
            $message .= $_;
    ...
    ## Read message body from STDIN
        ...
        $logger->info("Message input complete.");
    }
}
...
$SIG{'ALRM'} = sub { 
    $logger->error("Timeout while connecting to $conf{'server'}:$conf{'port'}");
    $logger->error("There was no response after $conf{'alarm'} seconds."); 
    $logger->info("Try specifying a different mail relay with the -s option.");
    quit("", 1);
};
alarm($conf{'alarm'}) if ($^O !~ /win/i);  ## alarm() doesn't work in win32;
...
if ( (!$SERVER) or (!$SERVER->opened()) ) {
    $logger->error("Connection attempt to $conf{'server'}:$conf{'port'} failed:");
    $logger->error("$@");
    $logger->info("Try specifying a different mail relay with the -s option.");
    quit("", 1);
}
...
$logger->debug("My IP address is: $conf{'ip'}");
...
if (SMTPchat('EHLO ' . $opt{'fqdn'}))   {
    ...
    $logger->info("NOTICE => EHLO command failed, attempting HELO instead");
    if (SMTPchat('HELO ' . $opt{'fqdn'})) { quit($conf{'error'}, 1); }
    if ( $opt{'username'} and $opt{'password'} ) {
        $logger->info("WARNING => The mail server does not support SMTP authentication!");
    }
}
else {
    ...
    if ($conf{'SMTPchat_response'} =~ /STARTTLS/) {
        $conf{'tls_server'} = 1;
        $logger->debug("The remote SMTP server supports TLS :)");
    }
    else {
        $conf{'tls_server'} = 0;
        $logger->debug("The remote SMTP server does NOT support TLS :(");
    }
...
#        if (! IO::Socket::SSL->start_SSL($SERVER, SSL_version => 'SSLv3 TLSv1')) {
        ...
        $logger->debug("=> TLS: Using cipher: ". $SERVER->get_cipher());
        $logger->debug("=> TLS session initialized :)");
    ...
    if ( $opt{'username'} and $opt{'password'} ) {
        if ($conf{'SMTPchat_response'} !~ /AUTH\s/) {
            $logger->info("NOTICE => Authentication not supported by the remote SMTP server!");
        }
        else {
            my $auth_succeeded = 0;
            my $mutual_method = 0;
            ...
            if ($auth_succeeded == 0 and $conf{'SMTPchat_response'} =~ /\bLOGIN\b/i) {
                $mutual_method = 1;
                $logger->debug("=> SMTP-AUTH: Using LOGIN authentication method");
                if (!SMTPchat('AUTH LOGIN')) {
                    if (!SMTPchat(base64_encode($opt{'username'}))) {
                        if (!SMTPchat(base64_encode($opt{'password'}))) {
                            $auth_succeeded = 1;
                            $logger->debug("=> User authentication was successful (Method: LOGIN)");
                        }
                    }
                }
                if ($auth_succeeded == 0) {
                    $logger->debug("=> SMTP-AUTH: LOGIN authenticaion failed.");
                }
            }
            ...
            if ($auth_succeeded == 0 and $conf{'SMTPchat_response'} =~ /\bPLAIN\b/i) {
                $mutual_method = 1;
                $logger->debug("=> SMTP-AUTH: Using PLAIN authentication method");
                if (SMTPchat('AUTH PLAIN ' . base64_encode("$opt{'username'}\0$opt{'username'}\0$opt{'password'}"))) {
                    $logger->debug("=> SMTP-AUTH: PLAIN authenticaion failed.");
                }
                else {
                    $auth_succeeded = 1;
                    $logger->debug("DEBUG => User authentication was successful (Method: PLAIN)");
                }
            }
            
            ## If none of the authentication methods supported by sendEmail were supported by the server, let the user know
            if ($mutual_method == 0) {
                $logger->warn("=> SMTP-AUTH: No mutually supported authentication methods available");
            }
...
foreach my $rcpt (@to, @cc, @bcc) {
    ...
    if (SMTPchat('RCPT TO:<' . $address . '>')) {
        $logger->warn("=> The recipient <$address> was rejected by the mail server, error follows:");
...
## If the message-format isn't raw, then build and send the message,
    ...
    if ($opt{'message-content-type'} eq 'html' or ($opt{'message-content-type'} eq 'auto' and $message =~ /^\s*(<HTML|<!DOCTYPE)/i) ) {
        $logger->debug("Setting content-type: text/html");
    ...
    ## Otherwise assume it's plain text...
        ...
        $logger->debug("Setting content-type: text/plain");
        print $SERVER "Content-Type: text/plain;$CRLF";
    }
    ## If they've specified their own content-type string...
    else {
        $logger->debug("Setting custom content-type: ".$opt{'message-content-type'});
    ...
    if (scalar(@attachments) > 0) {
        ...
        foreach my $filename (@attachments) {
            ...
            if ( ! -f $filename ) {
                $logger->error("The file [$filename] doesn't exist!  Email will be sent, but without that attachment.");
            }
            elsif ( ! -r $filename ) {
                $logger->error("Couldn't open the file [$filename] for reading: $!   Email will be sent, but without that attachment.");
            }
            else {
                $logger->debug("Sending the attachment [$filename]");
...
if ($logger->level() eq 10000) {
    $logger->debug("Generating a detailed exit message");
